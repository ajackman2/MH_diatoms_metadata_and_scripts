...
# select columns and group by the blade position (B, M, T)
# pivot longer
...
# format the box names
counts_position_long <- counts_position_long %>%
  mutate(box = paste0('box_', box))

# specify the box totals
...
colors = read.csv("scripts/mol_morph_comparison/taxaplot_colors/count_taxa_plot_colors.csv")
colors
...
colors = read.csv("scripts/mol_morph_comparison/taxaplot_colors/18S_taxaplot_colors.csv")
colors
...
colors = read.csv("scripts/mol_morph_comparison/taxaplot_colors/rbcl_taxaplot_colors.csv")
colors
