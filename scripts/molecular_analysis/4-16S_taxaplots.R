...
seagrass = readRDS("C:/Users/siobh/OneDrive - The University Of British Columbia/Project - Seagrass_Montague_2021-03-07/galliano_seagrass_2021and2023/imput_data/MH_16S_filtered001percent_notrarefied_phyloseq.rds")

seagrass@sam_data$read_depth_filtered = sample_sums(seagrass)
...
seagrassdf$relativeabundance = as.numeric(seagrassdf$asv_abundance)/as.numeric(seagrassdf$read_depth_filtered)
...
numcol <- length(unique(alldata$plotnames))-1

# 2. use microVis colour palettes for easily distinguishing taxa
newpal=distinct_palette(n = numcol, pal = "brewerPlus")

# 3. Extract hex colors
hex = as.data.frame(newpal)
...
# 5. Join color list and taxa names
topcolors = cbind(tops, hex)

# 6. for the "others" plot name, replace that with grey 90 (this is just an astetic thing)
topcolors[topcolors$plotnames == "Others",]$taxa_color <- "grey60"

# 7. Make an object R can pull form for the colors
...
##### MAKE THE TAXAPLOTS ######
alldata$LeafNumber[is.na(alldata$LeafNumber)]<-"water"
...
ggsave(filename = ("taxaplot_16S.pdf"), 
