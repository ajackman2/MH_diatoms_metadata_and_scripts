...
# read the data in
mol_18S <- read.csv("counting_data/06-11-24_blast_mol_18S")
...
mol2 = full_join(mol_RBCL, mol_18S)

mol2 = ddply(mol2, c("Genus"),
            summarise,
            sum.mol.ra = sum(ra),
            mean.mol.ra = mean(ra),
            n.mol.samples =length(unique(Row.names)))

mol2$in18S = ifelse(mol2$Genus %in% c(in18S), "yes", "no")
mol2$inRBCL = ifelse(mol2$Genus %in% c(inRBCL), "yes", "no")


###### combine data #####
all = full_join(mol2, diatom_data)
...
new_values <- tibble(
  Genus = "Parlibellus",
  sum.mol.ra = 8.138032e-02,
  mean.mol.ra =5.612436e-04,
  n.mol.samples = 29,
  in18S = 'no',
  inRBCL = 'yes',
  sum.sem.counts = 0,
...
plot_data$where_found <- ifelse(
  plot_data$in_counts == "yes" & plot_data$in_molecular == "yes", "in_both",
  ifelse(
    plot_data$in_counts == "yes" & plot_data$in_molecular == "no" , "in_morpho",
    ifelse(
      plot_data$in_counts == "no" & plot_data$in_molecular == "yes", "in_molecular", NA
      )
    )
  )
...
scatter_plot3 <- ggplot(plot_data, aes(x = sem_ra_rank, y = mol_ra_rank, color = where_found)) +
  ...
  geom_point(data = plot_data[plot_data$sum.mol.ra == 0 | plot_data$sem.ra == 0,], size = 2.5) +
  labs(x = "Morphological Rank", y = "Molecular Rank") +
  annotate("text", x = min(plot_data$sem_ra_rank) +5, y = max(plot_data$mol_ra_rank)-1,
           label = paste("p-value: 0.033", "\ny = 0.69x + 5.06"),
           hjust = 1, vjust = 1, size = 4) +
  scale_color_manual(labels = c("Both", "Molecular only", "Morphological only", "Morphological census only"),
                     values = c('goldenrod1', 'red3', 'blue3')) +
...
scatter_plot3 <- ggplot(plot_data, aes(x = sem_ra_rank, y = mol_ra_rank, color = where_found)) +
  ...
  geom_smooth(data = filtered_plot_data, method = 'lm') +
  geom_point(data = plot_data[plot_data$sum.mol.ra == 0 | plot_data$sem.ra == 0,], size = 2.5) +
  geom_text(aes(label = Genus), vjust = 0, hjust = 0, size = 3, check_overlap = FALSE) +
  labs(x = "Morphological Rank", y = "Molecular Rank") +
  annotate("text", x = min(plot_data$sem_ra_rank) + 8, y = max(plot_data$mol_ra_rank) - 1,
           label = paste("p-value: 0.078", "\ny = 0.55x + 6.39"),
           hjust = 1, vjust = 1, size = 4) +
  scale_color_manual(labels = c("Both", "Molecular only", "Morphological only", "Morphological census only"),
                     values = c('goldenrod1', 'red3', 'blue3', 'darkgreen')) +
  guides(color=guide_legend(title="Found in Morphological\nor Molecular Data?"))
...
new_values <- tibble(
  Genus = "Parlibellus",
  sum.mol.ra = 1.737321e-02,
  mean.mol.ra =1.198153e-04,
  n.mol.samples = 29,
  in18S = 'no',
  inRBCL = 'yes',
  sum.sem.counts = 0,
...
scatter_plot3 <- ggplot(plot_data, aes(x = sem_ra_rank, y = mol_ra_rank, color = where_found)) +
  ...
  geom_point(data = plot_data[plot_data$sum.mol.ra == 0 | plot_data$sem.ra == 0,], size = 2.5) +
  labs(x = "Morphological Rank", y = "Molecular Rank") +
  annotate("text", x = min(plot_data$sem_ra_rank) +6, y = max(plot_data$mol_ra_rank)-1,
           label = paste("p-value: 0.154", "\ny = 0.38x + 6.94"),
           hjust = 1, vjust = 1, size = 4) +
  scale_color_manual(labels = c("Both", "Molecular only", "Morphological only", "Morphological census only"),
                     values = c('goldenrod1', 'red3', 'blue3')) +
  guides(color=guide_legend(title="Found in Morphological\nor Molecular Data?")) 
...
lm <- lm(sem.ra ~ mean.mol.ra, data = filtered_plot_data)
summary(lm) # 0.07
...
cor(plot_data$mean.mol.ra, plot_data$sem.ra, method = 'pearson')
# replace NA with zero
plot_data$sem.ra[is.na(plot_data$sem.ra)] <- 0
plot_data$sum.mol.ra[is.na(plot_data$sum.mol.ra)] <- 0
...
lm <- lm <- lm(sem.ra ~ mean.mol.ra, data = filtered_plot_data)
summary(lm)
...
ggarrange(scatter_plot3, scatter_plot5, labels = c("A", "B"), ncol = 2)
...
# make a new col called in_molecular
ven_data2$in_molecular <- ifelse(ven_data2$in18S == "yes", "yes", "no")
