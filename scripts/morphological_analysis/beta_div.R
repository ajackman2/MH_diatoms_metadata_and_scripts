...
diatom_sums <- diatom_sums[, -c(30, 31)]
## run permanova with marginal effects 
m.perm= adonis2(diatom_sums[,-c(1:5)] ~ blade_position, 
                data=diatom_sums, method = "jaccard")
m.perm
# blade_position  2   1.0662 0.28968 2.4469  0.006 **
# Residual       12   2.6144 0.71032                 
# Total          14   3.6807 1.00000                         
...
m.pairwise
# pairs Df SumsOfSqs  F.Model        R2 p.value p.adjusted sig
# 1 B vs M  1 0.4914118 2.556181 0.2674898   0.095      0.285    
# 2 B vs T  1 0.6995742 3.755470 0.2944204   0.010      0.030   .
# 3 M vs T  1 0.2277177 2.900494 0.2660883   0.067      0.201       

## BETADISPERSON TEST ##
# calculate the distance
dist <- vegdist(diatom_sums[, 6:ncol(diatom_sums)], method = "jaccard")
# groups
groups <- diatom_sums$blade_position
# calculate the betadispersion within each region
dispers <- betadisp_result <- betadisper(dist, groups)
...
beta
# Df  Sum Sq  Mean Sq      F N.Perm Pr(>F)  
# Groups     2 0.16975 0.084877 4.1825    999  0.047 *
# Residuals 12 0.24352 0.020294                   
...
jac <- ggplot(diatom_sums, aes(x = NMDS1, y = NMDS2, color = blade_position, shape = leaf)) +
  geom_point(size = 4) +
  scale_color_manual(values=c("#55D3A6", "#5582D3", "#A655D3"))+
  annotate("text", x = -2.2, y = 1.3, label = "stress = 0.02499139 (Jaccard)", size=4, color="black")+
  annotate("text", x = -2.2, y = 1.2, label = "PERMANOVA: Section F= 2.4485, p= 0.005 **", size=4, color="black")+
  annotate("text", x = -2.2, y = 1.1, label = "No sd", size=4, color="black")+
  annotate("text", x = -2.2, y = 1, label = "BETADISPERSON F= 4.1775, p= 0.043 *", size=4, color="black") +
  labs(color = "Blade Position", shape = "Leaf")

ggarrange(bray, jac, labels = c("A", "B"))
